...
void Field::draw(SDL_Renderer *renderer)
{
    ...
    SDL_RenderDrawLine(renderer, centerX, TOP_MARGIN, centerX, height + TOP_MARGIN);
    SDL_RenderDrawLine(renderer, SIDE_MARGIN, TOP_MARGIN, SIDE_MARGIN, height + TOP_MARGIN);
    SDL_RenderDrawLine(renderer, SIDE_MARGIN + width, TOP_MARGIN, SIDE_MARGIN + width, height + TOP_MARGIN);

    // Retângulo esquerdo
    SDL_Rect leftRect = {SIDE_MARGIN, height / 4 + TOP_MARGIN, rec_width, rec_height};
    SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
    ...
    // Retângulo direito
    SDL_Rect rightRect = {SIDE_MARGIN + width - rec_width + 1, height / 4 + TOP_MARGIN, rec_width, rec_height}; // + 1 para corrigir falha na divisao de 450 por 8
    SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
    ...
    // Gol Robot 1 defende
    SDL_Rect leftGol = {0, TOP_MARGIN + 3 * gol_height / 2, gol_width, gol_height};
    ...
    // Gol Robot 2 defende
    SDL_Rect rightGol = {SIDE_MARGIN + width + 1, TOP_MARGIN + 3 * gol_height / 2, gol_width, gol_height};
